+++
title = "Software para controle de fiado"
omit_header_text = true
featured_image = "images/gh_banner-cashd.png"
toc = true
categories = ["implantação de tecnologia"]
+++

A venda no “fiado” ainda é bastante presente em nossa região, embora muitas vezes, não seja uma prática recomendada. Muitos comerciantes ainda se sentem presos à esta prática, por não se ver capazes de demonstrar nenhum diferencial em relação aos grandes concorrentes na região.

<!--more-->

## Como isso me afeta?

Saber exatamente o quanto está se comprometendo ao fazer uma venda à prazo pode evitar algumas armadilhas invisíveis da venda à prazo. Por isso, se você já aceita ou pretende aceitar fiado, é recomendado que conheça também o [[Como não quebrar vendendo à prazo]], já que isto afeta diretamente sua lucratividade e a capacidade de repor estoque.

Além do risco mais elevado de inadimplência do cliente, o fiado também pode afetar a sua operação, deixando o seu atendimento mais lento e tendo que manter contato para garantir que os clientes não esqueceram do saldo devedor.

## Como eu posso melhorar a minha situação?

Deixar de oferecer esta forma de pagamento nem sempre é uma opção viável no momento atual, mas ainda é possível minimizar os outros efeitos negativos que você pode sentir no dia-a-dia:

- É possível registrar estas compras e abatimentos de maneira rápida, com impacto mínimo no desempenho da sua operação;
- É possível acessar as informações de contato relevantes de seus clientes de maneira prática;
- Você pode encontrar facilmente o nome dos clientes que podem estar precisando de um lembrete.

Mesmo que você possa se organizar com sucesso usando o tradicional papel e caneta, você pode ter menos dificuldades e mais velocidade se optar por usar um *software* para isto, e eu posso oferecê-lo gratuitamente para você.

## Qual é o _software_?

O nome do _software_ que pode te ajudar é [[Cashd]], de código aberto e desenvolvido internamente, o Cashd é oferecido em duas distribuições: `cashd-server` e `cashd-local`, para mais informações sobre as distribuições, e para saber qual é a mais indicada para você, leia [a introdução do projeto](https://github.com/VFLins/cashd/blob/main/README.md).

Você pode instalar a versão mais recente do Cashd gratuitamente, basta seguir as [instruções de instalação](https://github.com/VFLins/cashd/wiki/Instalando-e-Atualizando#2-cashd-local), mas caso precise de ajuda para:

- Instalar a distribuição desejada,
- Treinamento, ou
- Suporte dedicado

Entre em contato, e nós podemos discutir o melhor orçamento para a sua situação e necessidades atuais.

