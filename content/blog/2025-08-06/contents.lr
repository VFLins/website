...
## 1 Introduzindo `FormHandler`

Três classes foram adicionadas para lidar com a criação dinâmica de formulários:

1. `FormField`, que representa um campo, o menor elemento de um formulário, e contém
um dos *inputs* que serão fornecidos pelo usuário;
2. `FormRow`, uma combinação de campos dispostos horizontalmente, ele deve controlar a
quantidade de colunas exibidas, a largura do formulário, e a largura máxima de cada um
de seus campos;
3. `FormHandler`, possui uma propriedade `full_contents` onde ficam os conteúdos de todos
...
### 1.1 Os campos do formulário

Uma instância da nova classe `FormField` retorna um `toga.Box` modificado, essencialmente
com três elementos:

1. Um rótulo (`label`), com um título para o campo;
2. Um *widget* de *input* (`input_widget`), onde o usuário vai inserir a informação;
3. Uma descrição opcional (`annotation`), um texto curto abaixo do `input_widget` com
...
### 1.3 O manipulador de formulários
...
Atualmente, para as necessidades do [Cashd](../../tag/Cashd), o único metadado que precisa ser acessado
...
## 2 Melhorias visuais e de experiência do usuário
...
### 2.2 Telas "Novo cliente" e "Configurações"
...
## 3 Migrando as duas versões do Cashd para um único repositório
...
> 📝 **Tarefa pendente:** Agregar os repositórios `cashd` e `cashd-cross` + Adaptar `cashd` com Taipy para agir como servidor.
